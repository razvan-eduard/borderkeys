...
| `assets/dict/ro_RO.bkd`, `en_US.bkd` | Compiled at build time from the word lists in `dictionaries` | GPL-3.0-or-later | Starter dictionaries: 1100 Romanian words and 540 English ones, with their most common word pairs. The lists are written in this repository rather than taken from a corpus, which is what turns "may we ship a dictionary" from an open question into an answered one — see 2.2. They are compiled by a Gradle task running the same `tools/build_dict.py` the tests use, so the packs in an APK are always exactly what the committed lists compile to and no binary is committed. |
...
| Asset | Position |
|---|---|
| A dictionary compiled from a real corpus | **Still not shipped, and this is the open question.** The starter dictionaries in 2.1 are small on purpose: they are this project's own writing, so their licence is this project's licence, and a thousand words is what one person can write and check. A dictionary that is actually good needs frequencies from a corpus, and the free ones — Wortschatz Leipzig under CC BY, OpenSubtitles frequency lists, the Hunspell `ro_RO` under GPL/LGPL/MPL — each come with terms that have to be read and recorded here before a blob is committed. Until then the starter is what ships and an import is how a better one arrives. |
