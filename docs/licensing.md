...
| `assets/emoji/emoji.txt`, `assets/emoji/emoji_names.txt` | Compiled by `tools/build_emoji.py` from **Unicode's `emoji-test.txt`** | **Unicode-3.0** (the data), GPL-3.0-or-later (the compiler) | 1,894 emoji in the groups and the order that file defines, which is the order it explicitly recommends for keyboard palettes, and the same file's English name for each, 39 KB, which is what the panel's search reads. 13 KB of code points; no images, because the glyphs come from the system font. The Unicode licence is permissive and BSD-shaped, and the attribution it asks for is this row and the entry in `REUSE.toml`. |
