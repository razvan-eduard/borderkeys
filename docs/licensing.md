...
| `assets/dict/{ro_RO,en_US,es_ES,fr_FR,de_DE,it_IT}.bkd` | Compiled at build time from the word lists in `dictionaries`, which are counted from the **Wortschatz Leipzig** corpora | **CC BY 4.0** (the counts), GPL-3.0-or-later (the format and the compiler) | Six languages, each counted with `tools/make_pack.py` from three Leipzig 1M-sentence corpora combined -- wikipedia, news and newscrawl -- after the original 18,000-word single-genre packs were found to be missing everyday words ("pepene", "ovăz" in Romanian) that a single formal-prose source never wrote down. Raised again from the first combined-corpus cut (which capped at 50,000-80,000 words) once that cap itself started crowding out real but less common words -- proper nouns, informal vocabulary -- that the same three corpora already had counts for. Compiled word counts as they actually land in the pack (format version 4, where a folded key carries every spelling that reaches it, each with its own frequency): English 128,217, Romanian 112,224, German 101,673, Italian 100,884, Spanish 99,859, French 100,782; up to 550,000 pairs and triples for English and Romanian, 500,000 for the other four. Each list also carries the names from 2.1.1, flagged in a third column; the counts above include them. Spellings that differ only by case are merged into one entry with their frequencies summed, because the keyboard decides case for itself; Romanian's three ways of writing the same accented letter (comma-below, cedilla, and none) are folded onto the standard spelling in the list itself by `tools/normalise_diacritics.py` before it is compiled, so a word's frequency is the sum across every one of them. All of it is published under CC BY 4.0; what is committed and shipped is **counts of words and pairs**, not the sentences they were counted from, and the attribution that licence asks for is this row, the entry in `REUSE.toml`, and the source link in the About screen. Compiled by a Gradle task running the same `tools/build_dict.py` the tests use, so the packs in an APK are always exactly what the committed lists compile to and no binary is committed. |
...
| `assets/offensive/{en-US,ro-RO,de-DE,fr-FR,es-ES,it-IT}.txt` | Written here | GPL-3.0-or-later | The words the "Block offensive words" switch keeps out of suggestions, corrections and learning: profanity and slurs with their inflections, one plain-text list per bundled language, roughly 80 to 130 words each. The selection is this project's own; it was checked for coverage against the LDNOOBW lists (CC BY 4.0) and the Romanian fork of them, and nothing was copied from either -- they were the wrong shape for a keyboard anyway, being content-moderation lists that mark ordinary anatomy and, in places, plain vocabulary. Each file carries its own header comment. |
| `assets/emoji/emoji.txt`, `assets/emoji/emoji_names.txt`, `assets/emoji/keywords/{en,de,es,fr,it,ro}.txt` | Compiled by `tools/build_emoji.py` from **Unicode's `emoji-test.txt`** and, for the keywords, **Unicode CLDR's `common/annotations` and `common/annotationsDerived`** in the six languages | **Unicode-3.0** (the data), GPL-3.0-or-later (the compiler) | 1,894 emoji in the groups and the order that file defines, which is the order it explicitly recommends for keyboard palettes, and the same file's English name for each, 39 KB, which is what the panel's search reads. Beside the names, the keywords CLDR's annotation files give every one of those emoji, one file per language, 81 to 132 KB each and 648 KB in all, of which the panel loads the languages switched on. 13 KB of code points; no images, because the glyphs come from the system font. The Unicode licence is permissive and BSD-shaped, and the attribution it asks for is this row and the entry in `REUSE.toml`. |
